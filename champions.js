...
function createChampCard(champ, version) {
  ...
  card.addEventListener('click', async () => {
    try {
      ...
      const response = await fetch(`https://ddragon.leagueoflegends.com/cdn/${version}/data/fr_FR/champion/${champ.id}.json`);
...
async function initChampionsPage() {
  try {
    ...
    searchInput.addEventListener('input', () => {
      ...
      champions.forEach(champ => {
        const card = champGrid.querySelector(`[alt="${champ.name}"]`).closest('.champ-card');
